...
exports.App = Framework.extend({
  ...
  addFramework: function(fw){
    var k;
    if(!fw) return;
    this._copyToFrameworkOptions.forEach(function(opt){
  ...
  addFrameworks: function(){
    ...
    args.forEach(this.addFramework.bind(this));
  },
  ...
  rootContent: function(htmlStylesheets, htmlScripts){
    return function(callback) {
      var html = this.get('_rootHTMLHead');
          
      var parseOrderedStylesheets = function(stylesheets){
        if (stylesheets) stylesheets.forEach(function(stylesheet) {
          html.push('<link href="' + this.get('urlPrefix') + stylesheet.get('url') + '" rel="stylesheet" type="text/css">');
        }.bind(this));
      }.bind(this);
      
      var parseOrderedScripts = function(scripts){
        if (scripts) scripts.forEach(function(script) {
          html.push('<script type="text/javascript" src="' + this.get('urlPrefix') + script.get('url') + '"></script>');
        }.bind(this));
      }.bind(this);
      
      if (!htmlStylesheets) {
        this._frameworks.getEach('orderedStylesheets').forEach(parseOrderedStylesheets);
      } else html.push(htmlStylesheets);
      html.push('</head>');
      
      html.push('<body class="%@ focus">'.fmt(this.get('theme')));
      if (this.get('htmlBody') !== null) html.push(this.get('htmlBody'));

      html.push('<script type="text/javascript">String.preferredLanguage = "%@";</script>'.fmt(this.buildLanguage));
      if(this.htmlScripts !== null) html.push(this.htmlScripts);

      if(!htmlScripts){
        this._frameworks.getEach('orderedScripts').forEach(parseOrderedScripts);
  ...
  build: function(callback){
    ...
    tools.async.forEach(this._frameworks, function(fw, cb) {
      fw.build(cb);
