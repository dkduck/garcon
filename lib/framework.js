...
exports.Framework = SC.Object.extend({
  ...
  init: function(){
    ...
    var pathsToExclude = [ /(^\.|\/\.|tmp\/|debug\/|test_suites\/|protocols\/|setup_body_class_names)/ ];
    this.pathsToExclude = pathsToExclude = this.pathsToExclude ? pathsToExclude.concat(this.pathsToExclude) : pathsToExclude;
  ...
  name: function(){
    return this.urlFor(this.get('path'));
  }.property('path'),
  ...
  afterFile: function(){
    return this.createVirtualFile('after.js', '; if ((typeof SC !== "undefined") && SC && SC.bundleDidLoad) SC.bundleDidLoad("%@");\n'.fmt(this.get('name')));
  }.property().cacheable(),
  ...
  computeDependencies: function(files, callback) {
    var requireMatcher = /require\(["'](.*?)["']\)/g;
    
    var fileMatcher = function(file, cb) {
      tools.qfs.readFile(file.path, function(err, data) {
        var match, path;

        if (err) throw err;
        file.deps = [];
        while (match = requireMatcher.exec(data)) {
          path = match[1];
          if (path.slice(-3) !== '.js') path += '.js';
          file.deps.push(files.findProperty('url', this.urlFor(tools.path.join(this.path, path))));
        }
        cb();
      }.bind(this));
    }.bind(this);

    if(files.length === 0) tools.log('no script files for fw: ' + this.get('name'));
    tools.async.forEach(files, fileMatcher, function() {
      callback(files);
    });
  },
  ...
  orderScripts: function(scripts, callback) {
    scripts = scripts.sort(function(a,b){
      ...
      return aPath.indexOf('core.js') >= 0 ? -1 : aPath.localeCompare(b.get('path'));
    });

    this.computeDependencies(scripts, function() {
      var orderedScripts = [];

      function insertFile(file) {
        var idx;
        if (!file) return -1;
        if ((idx = orderedScripts.indexOf(file)) >= 0) return idx;

        if (file.deps) file.deps.forEach(function(depFile) {
          var depFileIdx = insertFile(depFile);
          if (depFileIdx > idx) idx = depFileIdx;
        });
        if (idx < 0) idx = orderedScripts.length - 1;
        idx += 1;
        orderedScripts = orderedScripts.slice(0, idx).concat(file).concat(orderedScripts.slice(idx));
        return idx;
      }

      scripts.forEach(insertFile);
      callback(orderedScripts);
    }.bind(this));
  },
  
  _selectLanguageFiles: function(files) {
    var tmpFiles = {},
        buildLangShort = this.buildLanguage.toShortLanguage(),
        defaultLangShort = this.defaultLanguage.toShortLanguage(),
        file;
    ...
    files.forEach(function(file1) {
      ...
      if (file1Language === null || 
          ...
          file1Language === buildLangShort ||
          file1Language === this.defaultLanguage ||
          file1Language === defaultLangShort) {
        if (file2 === undefined) {
  ...
  _buildStylesheets: function(files){
    var handlers = 'ifModifiedSince contentType'.w(),
        combineStylesheets = this.get('combineStylesheets'),
        tmpFiles, file;
      
    if(this.minifyStylesheets) handlers.push('minify');
    //handlers.push(['rewriteStatic', "url('%@')"], 'join', 'file');
    handlers.push(['stylus', 'rewriteStatic', "url('%@')"], 'join'); // file is done by join if needed
  ...
  _buildScripts: function(files,callback){
    ...
    this.orderScripts(tmpFiles, function(orderedFiles){
      //tools.log('orderscripts for ' + me.get('name') + " calling cb..");
      var file;
      if(beforeFile) orderedFiles.unshift(beforeFile);
      if(afterFile) orderedFiles.push(afterFile);
      if(me.combineScripts){
        ...
        handler = HandlerSet.extend({ handlerList: "contentType file".w()});
        
        if(beforeFile){
          beforeFile.handler = handler.create();
          me.server.files[beforeFile.get('url')] = beforeFile;
        }
        if(afterFile){
          afterFile.handler = handler.create();
          me.server.files[afterFile.get('url')] = afterFile;
        }
        me.orderedScripts = orderedFiles;
      }
      callback();
    });
  },
  ...
  build: function(callback){
    ...
    this.scanFiles(function() {
      this._startBuilding.apply(this,arguments);
    }.bind(this));
  },
