...
exports.HandlerSet = SC.Object.extend({
  ...
  build: function(){
    return this.handlerList.map(function(name) {
      var k, params;
  ...
  handle: function(file,request,callback, handlers){
    ...
    if (!handlers) {
      if (!file.handlers) file.handlers = this.build();
    ...
    var f = function(stop){
      ...
      if((count < numHandlers) && !stop){ // if stop is given, it should start wrapping up immediately
        ...
        handlers[count].handle(file,request,f);
      }
      else wrapup(result);
    };
    ...
    var wrapup = function(ret){
      ...
      if(count >= 0) {
        ...
        handlers[count].finish(request,ret,wrapup);
      } 
      else {
